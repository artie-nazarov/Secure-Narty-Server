...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iters; i++) {
        ...
        // Launch thread A. This thread will try to pop off of an empty queue, and should hang
        pthread_create(&tA, NULL, threadA, args1);
        // Help ensure thread A starts
        sleep(DELAY);
        // Launch thread B. This thread will push onto an empty queue, unlocking thread A
